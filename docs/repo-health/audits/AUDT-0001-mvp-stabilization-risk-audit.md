...
updated_at: "2026-04-29 00:00:01 JST +0900"
...
|---|---|---|---|---|---|---|
| FINDING-001 | high | routed | Authenticated clients can bypass server-action price-log integrity because RLS mostly checks ownership while fields such as `normalized_price_yen`, `package_unit`, `price_tax_excluded_yen`, and `photo_path` are DB-callable. Evidence: `supabase/migrations/202603210001_init.sql:128`, `supabase/migrations/202603210001_init.sql:134`, `supabase/migrations/202603220001_comments_and_photos.sql:1`, `src/app/actions.ts:525`, `src/app/actions.ts:649`. | PLAN | PLAN-0002 | Routed to `PLAN-0002`; highest-risk item before wider public write usage. |
| FINDING-002 | high | routed | Write-side server actions have no automated coverage before the planned action-module refactor. Existing tests cover pricing, measurements, and demo data only, while risky write paths include create/update/delete log and vote/comment actions. Evidence: `src/lib/pricing.test.ts:4`, `src/lib/demo-data.test.ts:4`, `src/lib/measurements.test.ts:4`, `src/app/actions.ts:485`, `src/app/actions.ts:579`, `src/app/actions.ts:690`, `src/app/actions.ts:750`, `src/app/actions.ts:808`, `src/app/actions.ts:852`. | IMPL | IMPL-0001-02 | Routed to `IMPL-0001-02`; prioritize create/edit/delete validation, ownership/error paths, vote/comment branching, and cache/revalidation expectations. |
...
| FINDING-009 | medium | routed | Live read-model assembly and demo read-model assembly are duplicated, so demo/live parity can drift while tests still pass. Evidence: `src/lib/queries.ts:248`, `src/lib/queries.ts:283`, `src/lib/demo-data.ts:414`, `src/lib/demo-data.ts:459`, `src/lib/demo-data.test.ts:2`. | IMPL | IMPL-0001-02 | Routed to `IMPL-0001-02`; add pure mapper/fixture coverage so demo and live compare/feed rules share asserted semantics before `queries.ts` is split. |
| FINDING-010 | medium | routed | Live read-side query/view-model behavior is mostly untested; only demo compare ordering is covered. Evidence: `src/lib/demo-data.test.ts:5`, `src/lib/queries.ts:192`, `src/lib/queries.ts:248`, `src/lib/queries.ts:286`, `src/lib/queries.ts:308`, `src/lib/queries.ts:404`, `src/lib/queries.ts:666`, `src/lib/queries.ts:755`, `src/lib/queries.ts:907`, `docs/orientation/ARCHITECTURE.md:80`. | IMPL | IMPL-0001-02 | Routed to `IMPL-0001-02`; cover compare latest-log selection, feed sort modes, vote summary/viewer vote, comment ordering, and log-detail assembly. |
| FINDING-011 | medium | routed | Interaction-heavy frontend paths have no component/browser tests despite being high-regression surfaces. Evidence: `vitest.config.ts:10`, `package.json:23`, no `*.test.tsx` files found, `src/components/forms/price-log-form.tsx:176`, `src/components/forms/autocomplete-field.tsx:220`, `src/components/logs/log-vote-controls.tsx:63`, `src/components/forms/log-photo-input.tsx:61`, `src/components/compare/comparison-map.tsx:158`. | IMPL | IMPL-0001-02 | Routed to `IMPL-0001-02`; add a small RTL slice or explicitly defer frontend component coverage. |
...
| FINDING-015 | medium | routed | Verification is green locally but has no CI or coverage visibility, making regression protection process-dependent. Evidence: `package.json:5`, `vitest.config.ts:10`, `docs/repo-health/audits/audit-profile.md:28`. | IMPL | IMPL-0004-02 | Routed to `IMPL-0004-02`; decide CI/local-only/coverage gate before broad refactors. |
| FINDING-016 | low | routed | Log detail repeats item-level work to build latest-across-stores after already loading the same item logs and votes. Evidence: `src/lib/queries.ts:925`, `src/lib/queries.ts:927`, `src/lib/queries.ts:961`, `src/lib/queries.ts:703`, `src/lib/queries.ts:705`. | IMPL | IMPL-0004-01 | Routed to `IMPL-0004-01`; decide whether to fold into `IMPL-0001-03` after read-side coverage. |
| FINDING-017 | low | routed | `src/lib/queries.ts` is a read-side gravity well that combines Supabase access, cache policy, auth/viewer lookup, vote aggregation, comment assembly, and route snapshots. Evidence: `src/lib/queries.ts` is 1018 lines, `docs/orientation/CURRENT_STATE.md:78`, `docs/orientation/ARCHITECTURE.md:53`, `docs/repo-health/plans/PLAN-0001-documentation-and-architecture-cleanup/IMPL-0001-03-action-and-query-module-split.md:47`. | IMPL | IMPL-0001-03 | Routed to `IMPL-0001-03`; split only after `IMPL-0001-02` adds read-side characterization coverage. |
...
2. Continue with `IMPL-0001-02`, but broaden its test target list to include `FINDING-002`, `FINDING-009`, `FINDING-010`, and the highest-risk frontend behavior from `FINDING-011`.
3. Do not split `src/app/actions.ts` or `src/lib/queries.ts` until the coverage findings above are addressed or explicitly deferred.
4. Create a focused accessibility/verification slice for store creation and comment votes. `FINDING-003` is high because it blocks one contribution path for keyboard-only users.
5. Create a small diagnostic/evaluation plan for read-scale and map verification rather than treating those as immediate refactors.

## Follow-Ups

- Execute `IMPL-0001-02` before code splitting.
