...
updated_at: "2026-04-29 04:45:15 JST +0900"
...
| FINDING-003 | high | resolved | Physical store creation depends on a mouse/touch-only map pin even though physical stores require coordinates, blocking keyboard-only users from the required physical-store flow. Evidence: `src/app/actions.ts:383`, `src/components/forms/store-form.tsx:139`, `src/components/map/store-location-picker.tsx:26`. | none | none | Resolved by adding labeled latitude/longitude number inputs for physical stores in `src/components/forms/store-form.tsx`, keeping the Leaflet picker synced both ways, and enforcing latitude/longitude ranges in `storeSchema`. Component and schema tests cover keyboard coordinate entry, map synchronization, online coordinate clearing, and invalid coordinate ranges. |
| FINDING-004 | medium | resolved | Photo storage and DB mutations are not atomic, so failed DB writes can create orphaned photos or rows that point at removed photos. Evidence: `src/app/actions.ts:127`, `src/app/actions.ts:532`, `src/app/actions.ts:642`, `src/app/actions.ts:708`. | none | none | Resolved for current single-photo behavior by changing create/update/delete ordering in `src/app/actions.ts`: create removes a newly uploaded photo if row insert fails; replacement update removes a newly uploaded photo if row update fails and removes the old photo only after row update succeeds; delete removes the row first and then attempts storage cleanup. Remaining storage-cleanup failures are logged as accepted orphan risk for manual cleanup. |
| FINDING-005 | medium | resolved | Profile self-update RLS allows users to update any column on their own profile, while product behavior only intends username changes. Evidence: `supabase/migrations/202603230002_profiles_self_update.sql:1`, `src/lib/database.types.ts:27`, `src/app/actions.ts:331`. | none | none | Resolved by `supabase/migrations/202604290002_profile_rate_limit_constraints.sql`, which adds a profile self-update trigger that allows `public_name` changes while rejecting self-update changes to `id`, `email`, `display_name`, and `created_at`. Settings action behavior remains username-only. |
| FINDING-006 | medium | resolved | DB-backed rate limiting is count-then-insert rather than atomic, so parallel requests can exceed the intended action window. Evidence: `src/app/actions.ts:60`, `src/app/actions.ts:80`, `supabase/migrations/202603220003_action_rate_limits.sql:1`. | none | none | Resolved by `public.consume_action_rate_limit(action_name, max_events, window_seconds)`, which uses a transaction-level advisory lock per authenticated user/action, counts events inside the window, and inserts the event in the same function only when under the limit. `consumeRateLimit` now calls the RPC with existing action names, windows, and thresholds. |
| FINDING-007 | medium | accepted-risk | Public log and account pages depend on an unbounded all-log read, then sort/filter in application code. Evidence: `src/lib/queries.ts:594`, `src/lib/queries.ts:602`, `src/lib/queries.ts:782`, `src/lib/queries.ts:790`, `src/lib/queries.ts:794`, `src/app/logs/page.tsx:27`, `src/app/account/page.tsx:7`, `docs/repo-health/plans/PLAN-0004-read-scale-and-verification-gates/IMPL-0004-01-feed-scale-and-store-index-diagnostic.md`. | none | none | Accepted by owner: repo maintainer. Rationale: acceptable for MVP while `price_logs` remains in the low thousands and public/account traffic is light. Reopen before public launch or when `price_logs` reaches 5,000 rows, page server time exceeds about 1 second in staging/production, response payloads become visibly large, or account pages need reliable owner-only history. Simplest future fix: bounded database-backed feed pagination plus a separate `submitted_by` account query. |
| FINDING-008 | medium | deferred | Store detail has a store-only query path without a matching leading-column index; existing price-log indexes start with `item_id`. Evidence: `src/lib/queries.ts:566`, `src/lib/queries.ts:578`, `supabase/migrations/202603210001_init.sql:55`, `supabase/migrations/202603210001_init.sql:59`, `docs/repo-health/plans/PLAN-0004-read-scale-and-verification-gates/IMPL-0004-01-feed-scale-and-store-index-diagnostic.md`. | IMPL | future DB diagnostic | Deferred because this workspace has public Supabase env vars but no direct database URL, and `supabase status` reports no running local DB container. Run the documented `EXPLAIN (analyze, buffers)` query against local/staging/production-like data before adding a `store_id`-leading index. If the plan scans or reads heavily at realistic row counts, add the smallest likely migration: `(store_id, observed_at desc, created_at desc)`. |
...
| FINDING-012 | medium | resolved | Shared autocomplete is a custom combobox without enough active-option semantics for robust screen reader/keyboard behavior. Evidence: `src/components/forms/autocomplete-field.tsx:190`, `src/components/forms/autocomplete-field.tsx:289`. | none | none | Resolved by adding stable option/create-row IDs, `aria-activedescendant`, create-row keyboard activation, and tests for active option and create-action behavior in `AutocompleteField`. |
| FINDING-013 | medium | resolved | Comment vote controls expose glyph-only buttons without accessible names or pressed state. Evidence: `src/components/logs/log-vote-controls.tsx:107`, `src/components/comments/comment-vote-controls.tsx:84`. | none | none | Resolved by adding accessible vote labels and `aria-pressed` to comment and log vote buttons, with component tests covering pressed state. |
| FINDING-014 | medium | resolved | Map UI has known Leaflet lifecycle fragility but no automated or documented manual viewport/accessibility verification path. Evidence: `docs/ARCHITECTURE.md:186`, `src/components/compare/comparison-map.tsx:237`, `src/components/map/store-location-picker.tsx:49`, `src/components/stores/store-directory-map.tsx:49`, `package.json:5`. | none | none | Resolved by `docs/repo-health/debugging/map-manual-verification.md`, which records repeatable desktop/mobile checks for `/`, `/stores` directory map, and the signed-in `/stores` store picker, including marker render/selection/popup behavior, resize/orientation, geolocation denial expectations, keyboard tab order, and code-verified `?debug=1` usage. |
...
| FINDING-016 | low | resolved | Log detail repeats item-level work to build latest-across-stores after already loading the same item logs and votes. Evidence: `src/lib/queries.ts:925`, `src/lib/queries.ts:927`, `src/lib/queries.ts:961`, `src/lib/queries.ts:703`, `src/lib/queries.ts:705`, `docs/repo-health/plans/PLAN-0005-architecture-cleanup/IMPL-0005-01-action-and-query-module-split.md`. | none | none | Resolved by building `latestAcrossStores` in `getPriceLogDetail()` from the already-loaded `itemLogs` and `voteSummaries` via `buildCompareEntries()`, avoiding the extra `getComparisonSnapshot(log.item_id)` path while preserving the same read-model builder semantics. |
| FINDING-017 | low | deferred | `src/lib/queries.ts` is a read-side gravity well that combines Supabase access, cache policy, auth/viewer lookup, vote aggregation, comment assembly, and route snapshots. Evidence: `src/lib/queries.ts` is 1018 lines, `docs/orientation/CURRENT_STATE.md:78`, `docs/orientation/ARCHITECTURE.md:53`, `docs/repo-health/plans/PLAN-0005-architecture-cleanup/IMPL-0005-01-action-and-query-module-split.md:47`. | none | none | Partially addressed by moving pure read-model assembly, vote summarization, comment entry building, feed sorting, and relation stripping into `src/lib/query-read-models.ts`, reducing `src/lib/queries.ts` while keeping it as the stable snapshot/Supabase access facade. Reason: deeper snapshot splitting would add churn without a concrete route-level pain point yet. Revisit trigger: `src/lib/queries.ts` grows past about 900 lines again, a new route snapshot adds more cross-domain assembly, or a future audit finds concrete maintenance pain. |
...
| FINDING-019 | low | resolved | Form controls rely mostly on browser-default focus treatment despite custom UI surfaces, increasing manual accessibility verification risk. Evidence: `src/app/globals.css:874`. | none | none | Resolved by adding shared `.input`, `.select`, and `.textarea` `:focus-visible` styling in `src/app/globals.css`, using existing accent variables and no new color system. Manual verification is covered by `docs/repo-health/debugging/map-manual-verification.md` and the `IMPL-0003-03` checklist. |

## Recommendations

Recommended next order:

1. Before production rollout, run the live/staging DB checks called out by resolved backend findings and the deferred store-detail `EXPLAIN`.
2. Keep any further action/query splitting incremental and protected by the existing characterization tests.
