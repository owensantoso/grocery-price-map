...
| FINDING-016 | low | routed | Log detail repeats item-level work to build latest-across-stores after already loading the same item logs and votes. Evidence: `src/lib/queries.ts:925`, `src/lib/queries.ts:927`, `src/lib/queries.ts:961`, `src/lib/queries.ts:703`, `src/lib/queries.ts:705`, `docs/repo-health/plans/PLAN-0005-architecture-cleanup/IMPL-0005-01-action-and-query-module-split.md`. | IMPL | IMPL-0005-01 | Routed to `IMPL-0005-01`; `IMPL-0004-01` confirmed this belongs in the query-boundary split, where the detail read model can either reuse already-loaded item logs/vote summaries for `latestAcrossStores` or explicitly preserve the duplicate cached call. |
...
1. Execute `PLAN-0005` next so the documented action/query gravity wells can be split with read-scale decisions already recorded.
2. Before production rollout, run the live/staging DB checks called out by resolved backend findings and the deferred store-detail `EXPLAIN`.
