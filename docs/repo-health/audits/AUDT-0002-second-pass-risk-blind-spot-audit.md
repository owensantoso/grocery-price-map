...
|---|---|---|---|---|---|---|
| FINDING-001 | high | routed | Production can build and deploy without live Supabase env vars, silently falling back to demo-mode behavior. Evidence: `src/lib/supabase/env.ts:1`, `.github/workflows/ci.yml:36`, `README.md:49`. | IMPL | IMPL-0006-02 | Add a production deploy gate that fails or loudly blocks when required Supabase env vars are missing outside local/demo mode. |
| FINDING-002 | high | routed | Live/staging database migration and bypass verification is still not operationalized, including the two `20260429` hardening migrations. Evidence: `AUDT-0001#FINDING-001`, `AUDT-0001` recommendations, `README.md:56`, `docs/BACKEND_SCHEMA.md:224`, `supabase/migrations/202604290001_price_log_integrity.sql:1`, `supabase/migrations/202604290002_profile_rate_limit_constraints.sql:1`. | IMPL | IMPL-0006-02 | Create a DB release checklist that applies migrations in a clean staging/live-like project and verifies price-log trigger bypass checks, profile update checks, and rate-limit RPC behavior. |
| FINDING-003 | medium | routed | No production/staging deploy, rollback, or manual release runbook is present. Evidence: `docs/repo-health/audits/audit-profile.md:28`, `docs/repo-health/audits/audit-profile.md:29`, `README.md:21`, `README.md:80`. | IMPL | IMPL-0006-02 | Add a lightweight operations runbook covering deploy prerequisites, release verification, rollback, migration order, and owner responsibilities. |
| FINDING-004 | medium | routed | Production observability is too thin for triage: OAuth callback exchange failures redirect home without surfacing the reason, and storage/photo failures rely on `console.error` only. Evidence: `src/app/auth/callback/route.ts:8`, `src/app/auth/callback/route.ts:16`, `src/lib/price-log-photo-actions.ts:47`, `src/lib/price-log-photo-actions.ts:82`. | IMPL | IMPL-0006-02 | Add minimal diagnostic surfaces for auth/storage failures before production: structured logging, user-facing error routes where appropriate, and a short incident checklist. |
...
| FINDING-006 | medium | routed | Authenticated users can read all profile emails/display names through the `profiles` select policy, even though public surfaces are intended to use `public_name`. Evidence: `supabase/migrations/202603210001_init.sql:92`, `docs/BACKEND_SCHEMA.md:27`, `docs/DECISIONS.md:141`. | IMPL | IMPL-0006-03 | Restrict `profiles` reads to the owner row and keep public author labels on `public_profiles`. Verify settings/account flows still work. |
...
| FINDING-009 | medium | routed | Public photos lack an upload-time privacy warning and report/moderation controls, while store pages aggregate submitted photos publicly. Evidence: `supabase/migrations/202603220001_comments_and_photos.sql:66`, `src/app/stores/[storeId]/page.tsx:53`, `src/components/forms/price-log-form.tsx:332`, `docs/OPEN_QUESTIONS.md:15`. | IMPL | IMPL-0006-04 | Add an immediate public-photo warning in the upload flow. Report/moderation controls remain deferred to a moderation decision. |
| FINDING-010 | low | routed | Public text fields have no product or DB max length for comments, store notes, and log notes. Evidence: `src/lib/action-validation.ts:43`, `src/lib/action-validation.ts:66`, `src/lib/action-validation.ts:80`, `src/app/logs/[logId]/page.tsx:82`. | IMPL | IMPL-0006-03 | Add modest Zod limits and matching DB check constraints for public text fields. |
| FINDING-011 | low | routed | Username/profile updates are not rate-limited, unlike store/item/log/comment/vote actions. Evidence: `src/app/actions.ts:93`, `src/lib/action-helpers.ts:16`. | IMPL | IMPL-0006-03 | Add a `profile-update` or `username-update` action rate limit. |
...
| FINDING-017 | medium | routed | Non-compare maps lack the resize/orientation lifecycle hardening used by the compare map, even though the manual checklist expects resize/orientation verification for all map surfaces. Evidence: `src/components/compare/comparison-map.tsx:85`, `src/components/stores/store-directory-map.tsx:47`, `src/components/map/store-location-picker.tsx:60`, `docs/repo-health/debugging/map-manual-verification.md:40`. | IMPL | IMPL-0006-04 | Add shared map resize invalidation behavior or document why only the compare map needs it, then update manual verification notes. |
| FINDING-018 | medium | routed | The "create missing item/store" flow from the price-log form drops draft state except the newly created entity name. Evidence: `src/components/forms/price-log-form.tsx:217`, `src/components/forms/price-log-form.tsx:237`, `src/app/actions.ts:82`, `src/app/actions.ts:227`, `src/app/prices/new/page.tsx:48`. | IMPL | IMPL-0006-05 | Preserve price-log draft state across item/store creation, or make the workflow explicitly separate so users do not lose entered price/date/photo/notes data. |
| FINDING-019 | low | routed | Mobile header search overlay lacks an Escape/keyboard close path. Evidence: `src/components/navigation/header-search.tsx:42`, `src/components/navigation/account-menu.tsx:24`, `src/app/globals.css:1603`. | IMPL | IMPL-0006-04 | Add Escape handling and keyboard-close behavior consistent with the account menu. |
