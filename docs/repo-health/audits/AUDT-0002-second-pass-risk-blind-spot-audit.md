...
| FINDING-004 | medium | resolved | Production observability is too thin for triage: OAuth callback exchange failures redirect home without surfacing the reason, and storage/photo failures rely on `console.error` only. Evidence: `src/app/auth/callback/route.ts:8`, `src/app/auth/callback/route.ts:16`, `src/lib/price-log-photo-actions.ts:47`, `src/lib/price-log-photo-actions.ts:82`. | none | none | Resolved by `IMPL-0006-02`: auth callback failures redirect to the existing sign-in error surface with structured `auth_callback_*` logs, photo failures emit structured `price_log_photo_*` events, and `docs/repo-health/operations/incident-triage-checklist.md` documents provider-neutral triage. |
